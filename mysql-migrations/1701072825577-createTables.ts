...
export class CreateTables1701072825577 implements MigrationInterface {
    name = 'CreateTables1701072825577';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `CREATE TABLE \`Users\` (\`id\` varchar(36) NOT NULL COMMENT '사용자 id', \`username\` varchar(50) NOT NULL COMMENT '닉네임', \`email\` varchar(50) NOT NULL COMMENT '이메일', \`password\` varchar(255) NOT NULL COMMENT '비밀번호', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, UNIQUE INDEX \`IDX_3c3ab3f49a87e6ddb607f3c494\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
            `CREATE TABLE \`Bids\` (\`id\` int NOT NULL AUTO_INCREMENT, \`item_id\` int NOT NULL COMMENT '경매 상품 idx', \`user_id\` varchar(36) NOT NULL COMMENT '사용자 id', \`bid_amount\` int NOT NULL COMMENT '입찰 금액', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`Auction_items\` (\`id\` int NOT NULL AUTO_INCREMENT, \`c_code\` char(4) NULL COMMENT '카테고리 분류 코드', \`title\` varchar(255) NOT NULL COMMENT '경매 등록 상품 이름', \`content\` text NOT NULL COMMENT '경매 등록 상품 설명', \`image\` text NULL COMMENT '경매 등록 상품 이미지 url', \`user_id\` varchar(36) NOT NULL COMMENT '사용자 id', \`status\` tinyint NOT NULL COMMENT '경매 진행중 여부' DEFAULT '0', \`likes\` int NOT NULL COMMENT '찜 수' DEFAULT '0', \`start_datetime\` datetime NOT NULL COMMENT '경매 시작 datetime', \`end_datetime\` datetime NOT NULL COMMENT '경매 종료 datetime', \`start_price\` int NOT NULL COMMENT '시작 입찰 가격' DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
